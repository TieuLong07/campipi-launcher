  ...
  return (
    <aside>
      ...
      <div className="user-card" data-testid="user-card" onClick={() => setPopoverOpen((o) => !o)}>
        ...
        <div className="info">
          <div className="name">{account?.username ?? 'Chưa đăng nhập'}</div>
          <div className="type">{account?.type ?? '—'}</div>
        </div>
      </div>
      {popoverOpen && (
        <div className="user-popover open" data-testid="user-popover" onClick={(e) => e.stopPropagation()}>
          <div className="item" data-testid="popover-manage" onClick={() => { setPopoverOpen(false); onOpenAccountModal(); }}>
            <IconEdit /> Quản lý tài khoản
          </div>
