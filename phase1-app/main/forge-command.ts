...
export async function buildForgeCommand(opts: BuildOptions): Promise<BuiltCommand> {
  ...
  // XMCL generateArguments expects profile with name, uuid, accessToken
  ...
  // XMCL returns full argv including java path — use as-is
  const argv = [...xmclArgs];
  ...
  const mainClass = version.mainClass || 'cpw.mods.bootstraplauncher.BootstrapLauncher';
